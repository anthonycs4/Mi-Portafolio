...
interface TechSticker {
  ...
  name: string;
  category: Category;
  icon: ReactNode; // emoji or simple icon
...
export function TechStackSection() {
  ...
  const categories: (Category | 'Todo')[] = ['Todo', 'Frontend', 'Backend','Database', 'Mobile', 'Cloud', 'Tools'];
  
  const filteredStack = activeCategory === 'Todo' 
    ? techStack 
    : techStack.filter(tech => tech.category === activeCategory);

  return (
    <section className="relative w-full max-w-[1200px] min-h-[700px] mx-auto overflow-visible rounded-2xl pb-12">
      {/* Dark desk/table background */}
      ...
      <div className="absolute top-0 left-1/4 w-96 h-96 bg-neon-cyan/5 rounded-full blur-[100px] pointer-events-none" />
      <div className="absolute bottom-0 right-1/4 w-96 h-96 bg-neon-purple/5 rounded-full blur-[100px] pointer-events-none" />
      ...
      <div className="relative z-10 h-full flex flex-col p-8">
        ...
        {/* Stickers container - changed overflow-hidden to overflow-visible */}
        <div className="relative flex-1 min-h-[550px] overflow-visible rounded-lg">
          ...
          {/* Optional connecting lines between related techs */}
...
function TechSticker({ tech, isHovered, onHover, onLeave }: TechStickerProps) {
  return (
    ...
    >
      ...
      {isHovered && (
        <div 
          className="absolute bottom-full left-1/2 -translate-x-1/2 mb-4 w-64 bg-space-light backdrop-blur-xl border border-neon-cyan/50 rounded-lg p-4 shadow-[0_0_30px_rgba(0,240,255,0.3)] animate-fade-in"
